...
public class TC_ModifyUserAPI extends TestInit {
	...
	/**
	 * Test Case: TC07_ModifyUserProfile Test Type: Positive Description: To verify
	 * that user is able to modify existing user in system through API
	 *
	...
	public void TC07_ModifyUserProfile() throws AutomationException {
		...
		ExtentTestManager.startTest("Modify User Profile",
				"To verify that user is able to modify existing user in system through API");

		List<User> userList = (List<User>) UserProfileService.init().getUserProfiles().getResponse();

		User userToModify = userList.get(1);
		userToModify.setFirstname("ModifiedFirstName" + Randomizer.randomNumeric(5));

		User createdUser = (User) UserProfileService.init().modifyUserProfiles(userToModify).getResponse();
	...
	/**
	 * Test Case: TC08_ModifyUserProfile Test Type: Negative Description: To verify
	 * that user is able to modify user in system if userid provided does not exist
	 * through API
	 *
	 * @throws AutomationException
	 */
	...
	public void TC08_ModifyUserProfile() throws AutomationException {
		...
		ExtentTestManager.startTest("Modify User Profile",
				"To verify that user is able to modify user in system if userid provided does not exist through API");
		...
		UserProfileService.init().isNegativeTest(HttpStatus.INTERNAL_SERVER_ERROR).modifyUserProfiles(userToModify);
